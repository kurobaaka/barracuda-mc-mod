...
public class ElectricSmelterScreenHandler extends ScreenHandler {
    private final PropertyDelegate propertyDelegate;
    private final ElectricSmelterBlockEntity blockEntity;
    private final ScreenHandlerContext context;

    public ElectricSmelterScreenHandler(int syncId, PlayerInventory inventory, PacketByteBuf buf) {
        this(syncId, inventory, Objects.requireNonNull(inventory.player.getWorld().getBlockEntity(buf.readBlockPos())),
                new ArrayPropertyDelegate(4));
    }

    public ElectricSmelterScreenHandler(int syncId, PlayerInventory playerInventory,
                                        BlockEntity blockEntity, PropertyDelegate arrayPropertyDelegate) {
        super(ModScreenHandlerType.ELECTRIC_SMELTER, syncId);

        this.propertyDelegate = arrayPropertyDelegate;
        this.blockEntity = ((ElectricSmelterBlockEntity) blockEntity);
    ...
    private void addBlockEntityInventory() {
        ...
        addSlot(new Slot(inventory.getInventory(0), 0, 44, 30));
        addSlot(new Slot(inventory.getInventory(1), 0, 100, 30));
        addSlot(new Slot(inventory.getInventory(2), 0, 8, 12));
        addSlot(new Slot(inventory.getInventory(2), 1, 8, 30));
        addSlot(new Slot(inventory.getInventory(2), 2, 8, 48));
    }
    ...
    public boolean canUse(PlayerEntity player) {
        return canUse(player, ModBlocks.ELECTRIC_SMELTER);
    }
